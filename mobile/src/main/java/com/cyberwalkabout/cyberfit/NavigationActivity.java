    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (loader.getId() == ContentProviderAdapter.LOADER_USER) {
            if (data != null && data.moveToFirst()) {
                String displayName = data.getString(data.getColumnIndex(UserTable.COLUMN_DISPLAY_NAME));
                String username = data.getString(data.getColumnIndex(UserTable.COLUMN_USERNAME));
                String imageUri = data.getString(data.getColumnIndex(UserTable.COLUMN_IMAGE_URI));

                boolean isMale = true;
                if (!data.isNull(data.getColumnIndex(UserTable.COLUMN_IS_MALE))) {
                    isMale = data.getInt(data.getColumnIndex(UserTable.COLUMN_IS_MALE)) == 1;
                }
