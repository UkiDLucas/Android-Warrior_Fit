...
import com.warriorfitapp.mobile.MapExerciseScreen;
...
public class TrainingProgramDetailsFragment extends ListFragment implements AdapterView.OnItemClickListener, IFilterable, LoaderManager.LoaderCallbacks<Cursor>, CompoundButton.OnCheckedChangeListener {
    ...
    private com.warriorfitapp.model.v2.Program program;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_share) {
            new AsyncTask<Long, Void, com.warriorfitapp.model.v2.Author>() {
                @Override
                protected com.warriorfitapp.model.v2.Author doInBackground(Long... params) {
                    return contentProviderAdapter.getAuthorById(getActivity(), params[0]);
                }

                @Override
                protected void onPostExecute(com.warriorfitapp.model.v2.Author author) {
    ...
    private class ExercisesAdapter extends ResourceCursorAdapter {
        ...
        public void bindView(final View view, Context context, Cursor cursor) {
            ...
            final String id = cursor.getString(cursor.getColumnIndex(com.warriorfitapp.db.sqlite.schema.table.ExerciseTable.COLUMN_ID));
            String name = cursor.getString(cursor.getColumnIndex(com.warriorfitapp.db.sqlite.schema.table.ExerciseTable.COLUMN_NAME));
            String description = cursor.getString(cursor.getColumnIndex(com.warriorfitapp.db.sqlite.schema.table.ExerciseTable.COLUMN_DESCRIPTION));
            if (!TextUtils.isEmpty(description)) {
                description = description.trim().replaceAll(" +", " ").replaceAll("\\n|\\r", "");
            }

            boolean isFavorite = !TextUtils.isEmpty(id) && id.equals(cursor.getString(cursor.getColumnIndex(com.warriorfitapp.db.sqlite.schema.table.ExerciseTable.ALIAS_FAVORITE_EXERCISE_ID)));
            ...
            String youtubeId = cursor.getString(cursor.getColumnIndex(com.warriorfitapp.db.sqlite.schema.table.ExerciseTable.COLUMN_YOUTUBE_ID));
